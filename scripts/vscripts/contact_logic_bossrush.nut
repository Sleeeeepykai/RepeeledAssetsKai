...
{
    IsLowGravity = false
    SetLowGravity = function(isLowGrav = true) {
        Contact.IsLowGravity = isLowGrav
        for (local i = 1, player; i <= MaxClients().tointeger(); i++)
            if (player = PlayerInstanceFromIndex(i), player) {
                if(IsLowGravity == true) player.SetGravity(0.7)
                else player.SetGravity(1.0)
            }
    }
    Cleanup = function()
    {
    ...
    OnScriptHook_OnTakeDamage = function(params)
    {
        ...
        if ( ent.GetClassname() == "prop_dynamic" && ent.GetName().find("drone")!=null)
        {
    ...
    OnGameEvent_player_death = function(params)
    {
        ...
        if(player.GetTeam() == 3) {
            ...
            EntFireByHandle(player,"$ShowToAll",null,0,player,player)
            player.KeyValueFromString("rendermode", "1")
            EntFireByHandle(player, "EnableDamageForces", null, 0.0, null, null)
        }
    ...
    OnGameEvent_player_spawn = function(params)
    {
        ...
        if (player != null && player.GetTeam() == 3) {
            EntFireByHandle(player, "RunScriptCode", "Contact.botSetup(self)", 0.05, null, null)
        }
    }
...
Contact.botSetup <- function(player) {
    // local tags = {}
    // player.GetAllBotTags(tags)
    // foreach(key,val in tags ){
    //     ClientPrint(null, 3, val.tostring())
    // }
    local bot = player
    if (player.HasBotTag("spyboss")) {
        Contact.spyBoss = bot
        bot.ValidateScriptScope()
        EntFireByHandle(bot, "SetLightingOrigin", "manninblacklightingref", 0.5, null, null)

        local scope = bot.GetScriptScope()
        for (local wearable = bot.FirstMoveChild(); wearable != null; wearable = wearable.NextMovePeer())
        {
            // if (wearable.GetClassname() == "tf_wearable"){
            //     EntFireByHandle(wearable, "SetLightingOrigin", "manninblacklightingref", 0.5, null, null)
            //     //EntFireByHandle(wearable, "$SetKey$renderfx", "15", 0, null, null)
            //     EntFireByHandle(wearable, "$SetKey$renderamt", "245", 0.5, null, null)
            //     EntFireByHandle(wearable, "$SetKey$rendermode", "1", 0.5, null, null)
            //     ClientPrint(null,3,"testmessage " + wearable.GetModelName().tostring())
            // }
            if (wearable.GetClassname() != "tf_wearable"||wearable.GetModelName().find("chicago_overcoat.mdl")==null)
                continue;
            EntFireByHandle(wearable, "SetLightingOrigin", "manninblacklightingref", 0.1, null, null)
            //EntFireByHandle(wearable, "$SetKey$renderfx", "15", 0, null, null)
            EntFireByHandle(wearable, "$SetKey$renderamt", "200", 0.1, null, null)
            EntFireByHandle(wearable, "$SetKey$rendermode", "1", 0.1, null, null)
            //ClientPrint(null,3,"testmessage " + wearable.tostring())
        }
    }
    if (player.HasBotTag("medicboss")) {
        Contact.medBoss = bot
    }
    if (player.HasBotTag("engidrone")) {
        ...
        bot.Teleport(true, Vector(-13203, 3964, 37), false, QAngle(0,0,0), false, Vector(0,0,0))
        // EntFireByHandle(bot, "RunScriptCode", "self.Teleport(true, Vector(-13203, 3964, -100), false, QAngle(0,0,0), false, Vector(0,0,0))", 1, null, null)
        // EntFireByHandle(bot, "RunScriptCode", "self.Teleport(true, Vector(-13203, 3964, -100), false, QAngle(0,0,0), false, Vector(0,0,0))", 3, null, null)
    ...
    if (player.HasBotTag("engiboss")) {
        if(debugzz) ClientPrint(null, 3, "engiboss spawned")
        bot.ValidateScriptScope()
        local scope = bot.GetScriptScope()
        bot.SetGravity(1.0)
        ...
        if(Contact.engiDroneLastLoc == null) Contact.engiDroneLastLoc = Vector(-13203, 3964, -100)
        local location = Contact.engiDroneLastLoc + Vector(0, 0, 500)
        location.z = Min(location.z,1000)
        Contact.engiBoss = bot
        Contact.CreatePlayerWearable(bot, "models/workshop/player/items/all_class/bak_teufort_knight/bak_teufort_knight_engineer.mdl", true)
        Contact.CreatePlayerWearable(bot, "models/player/items/scout/scout_henchboy_hat.mdl", true)
        //Contact.CreatePlayerWearable(bot, "models/weapons/c_models/c_engineer_bot_gunslinger.mdl", true)


        //Contact.CreatePlayerWearable(bot, "models/player/items/engineer/hwn_engineer_misc1.mdl", true)
        //Contact.CreatePlayerWearable(bot, "models/workshop/player/items/engineer/hwn2022_dustbowl_devil/hwn2022_dustbowl_devil.mdl", true)
...
Contact.SetupDrone <- function(bot) {
    ...
    bot.ValidateScriptScope()
    bot.GetScriptScope().takeDamageIfNearbyFix <- Contact.takeDamageIfNearbyFix
    AddThinkToEnt(bot, "takeDamageIfNearbyFix")
}
...
Contact.SetEngiUfoTrigger <- function(dummy) {
    ...
    AddThinkToEnt(dummy, "Contact.EngiUfoTriggerThink")
...
::contactRingRegister <- function(beam) {
    ...
    AddThinkToEnt(beam, "contactRingThink")
}
...
Contact.tankModelFix <- function(tank) {
    try {
        local model = "models/mvm_ufo/ufo_core.mdl"
...
Contact.engiufotankSetup <- function(tank) {
...
Contact.KillEngiDrone <- function()
{
    ...
    if (Contact.lock) {
        if(Contact.engiDrone != null && Contact.engiDrone.IsValid() && Contact.engiDrone.GetClassname() == "player" && NetProps.GetPropInt(Contact.engiDrone, "m_lifeState") == 0) {
        	EntFire("tf_gamerules", "RunScriptCode", "Contact.KillEngiDrone()", 0.1, null)
        }
    }
    Contact.lock = true
    if(Contact.engiDrone != null && Contact.engiDrone.IsValid() && Contact.engiDrone.GetClassname() == "player" && NetProps.GetPropInt(Contact.engiDrone, "m_lifeState") == 0) {
...
// some functions borrowed from popextensions
Contact.SetParentLocalOrigin <- function(child, parent, attachment = null) {
	if (typeof child == "array")
		foreach(i, childIn in child)
			Contact.SetParentLocalOriginDo(childIn, parent, attachment)
	else
		Contact.SetParentLocalOriginDo(child, parent, attachment)
}
Contact.SetParentLocalOriginDo <- function(child, parent, attachment = null) {
...
Contact.CreatePlayerWearable <- function(player, model, bonemerge = true, attachment = null, autoDestroy = true) {
	...
	Contact.SetParentLocalOrigin(wearable, player, attachment)
...
Contact.LaserIndicator <- function(duration){
    ...
    Contact.SetParentLocalOrigin(wep_marker, wep, "muzzle")
